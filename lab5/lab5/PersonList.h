...
public: 
	...
	void AddAt(T1 *item, int index)
	{
		...
		if (!_head && !index || GetCount() && index == GetCount()) // ≈сли список пустой или эленет прос€т добавить в хвост
		{
			Add(item);
			return;
		}
		...
		else if (GetCount() && index == GetCount()-1) // ѕрос€т добавить на место последнего элемента
		{
